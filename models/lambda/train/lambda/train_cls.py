...
def get_data(args):
	train_data = pd.read_csv(args.train_data_path).groupby('sentiment').apply(lambda x: x.sample(int(10)))
	val_data = pd.read_csv(args.val_data_path).groupby('sentiment').apply(lambda x: x.sample(int(10)))


	labels = sorted(list(set(train_data['sentiment'].unique().tolist() + val_data['sentiment'].unique().tolist())))
	label2id = {label: i for i, label in enumerate(labels)}
	train_data['sentiment'] = train_data['sentiment'].apply(lambda x: label2id[x])
	val_data['sentiment'] = val_data['sentiment'].apply(lambda x: label2id[x])
